...
private:
  ...
  /* the previous frame's state as received, before overlays; overlay
     painting copies-on-write, so rows it touched no longer compare
     pointer-equal with local_framebuffer's */
  Terminal::Framebuffer network_framebuffer;
